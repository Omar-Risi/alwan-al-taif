...
export default function EditNewsPage() {
  ...
  useEffect(() => {
    async function fetchNews() {
      try {
        ...
        if (!res.ok) {
          let errorMessage = 'الخبر غير موجود';
          try {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Then update the news with all images
      const newsId = Array.isArray(params.id) ? params.id[0] : params.id;
        ...
        <div className="bg-white/50 backdrop-blur rounded-lg shadow p-6 border border-primary/20">
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {pendingImages.length > 0 && (
                <div className="mt-4">
                  <p className="text-sm font-medium text-gray-700 mb-2">
                    الصور المحددة ({pendingImages.length}) - سيتم رفعها عند حفظ التعديلات
                  </p>
            ...
            <div className="flex gap-4">
              ...
              >
                {uploading ? 'جاري رفع الصور...' : loading ? 'جاري الحفظ...' : 'حفظ التعديلات'}
              </button>
              <Link
                href="/dashboard/news"
            ...
            {pendingImages.length > 0 && (
              <p className="text-sm text-blue-600">
                ملاحظة: سيتم رفع {pendingImages.length} صورة عند الضغط على حفظ التعديلات
              </p>
            )}
          </form>
        </div>
